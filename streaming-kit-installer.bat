...
SET REPO_DOWNLOAD_URL=https://github.com/kandi1clickkits/streaming-anomaly-detection/releases/download/v1.0.0/streaming-anomaly-detection.zip
SET REPO_DEPENDENCIES_URL=https://github.com/kandi1clickkits/streaming-anomaly-detection/raw/main/requirements.txt
SET REPO_NAME=streaming-anomaly-detection.zip
SET EXTRACTED_REPO_DIR=streaming-anomaly-detection
SET NOTEBOOK_NAME=streaming-anomaly-detection.ipynb
SET NOTEBOOK_NAME1=Inference.ipynb
...
CD /D !WORKING_DIR!\!KIT_NAME!

REM CALL START !WORKING_DIR!\!VIRTUALENV_NAME!\Scripts\activate.bat & python kafkaproducer.py
REM CALL START !WORKING_DIR!\!VIRTUALENV_NAME!\Scripts\activate.bat & python kafkaconsumer.py

CALL START !WORKING_DIR!\!VIRTUALENV_NAME!\Scripts\python.exe kafkaproducer.py
CALL START !WORKING_DIR!\!VIRTUALENV_NAME!\Scripts\python.exe kafkaconsumer.py
EXIT /B %ERRORLEVEL%
...
IF EXIST !WORKING_DIR!\%EXTRACTED_REPO_DIR%\ (
	...
	REM CALL :Install_java_and_modules
    TITLE Installing %KIT_NAME% kit 100%% xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    timeout 1  >nul
	for /f %%A in ('copy /Z "%~dpf0" nul') do set "CR=%%A"
	<nul set/p"=->!CR!"
...
:Download_kafka
CALL :LOG "Downloading kafka... " >> !WORKING_DIR!\log.txt 2>&1
curl https://downloads.apache.org/kafka/3.4.0/kafka_2.13-3.4.0.tgz --output kafka.tgz >> !WORKING_DIR!\log.txt 2>&1
tar -xzf kafka.tgz -C !KAFKA_INSTALL_LOCATION!
DEL kafka.tgz
cd !KAFKA_INSTALL_LOCATION! 
REN !KAFKA_INSTALL_LOCATION!\kafka_2.13-3.4.0 kafka

IF ERRORLEVEL 1 (
	CALL :LOG "Error with Downloading kafka... "
    EXIT /B 1
)

CALL :LOG "Download of Kafka completed... "

CALL :LOG "Attempting to run Zookeeper and Kafka... "

REM cd %KAFKA_HOME%
CALL START %KAFKA_HOME%\bin\windows\zookeeper-server-start.bat %KAFKA_HOME%\config\zookeeper.properties 

timeout /t 20 /nobreak > NUL 

CALL START %KAFKA_HOME%\bin\windows\kafka-server-start.bat %KAFKA_HOME%\config\server.properties

timeout /t 10 /nobreak > NUL 

CALL START %KAFKA_HOME%\bin\windows\kafka-topics.bat --create --topic potential-drowning --bootstrap-server localhost:9092  --partitions 3 --replication-factor 1 --if-not-exists 
